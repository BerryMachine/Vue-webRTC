  ...
  export default {
    ...
    methods: {
      handleCheckIn: function() {
        const roomRef = db.collection('users').doc(this.$route.params.hostID).collection('rooms').doc(this.$route.params.roomID)
        roomRef.get().then(doc => {if(doc.exists) {roomRef.collection('attendees').doc(this.user.uid).set({displayName: this.displayName, createdAt: firebase.firestore.FieldValue.serverTimestamp()}).then(() => this.$router.push(`/chat/${route.params.hostID}/${this.$route.params.roomID}`))
          }
        })
      }
    },
    ...
    mounted() {
      ...
      //Getting Room Name
      db.collection('users').doc(this.$route.params.hostID).collection('rooms').doc(this.$route.params.roomID).get().then(doc => {if (doc.exists) {this.roomName = doc.data().name} else {this.$router.replace('/')
        }
      })
    }
  }
